...
public class MetodosDAO {
    ...
    public ResultSet selectQuestoes(String dificuldade, String tema) {

        try {
            String sql = "SELECT * FROM perguntas A left JOIN dificuldades B ON A.Dificuldade = B.id left JOIN temas C ON A.Tema = C.id where C.tema = ? and B.Dificuldade = ?";
            PreparedStatement stmt = con.prepareStatement(sql);
            stmt.setString(1, dificuldade);
            ...
            JOptionPane.showMessageDialog(null, "selectQuestao " + erro);
        }
        return null;
    }
}
